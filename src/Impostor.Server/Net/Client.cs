...
internal class Client(
    ...
    : ClientBase(name, gameVersion, language, chatMode, platformSpecificData, connection)
{
    ...
    public override async ValueTask<bool> ReportCheatAsync(CheatContext context, CheatCategory category, string message)
    {
        ...
        {
            logger.LogWarning("Unknown cheat category {Category} was used when reporting", category);
            return true;
        }
    ...
    public override async ValueTask HandleDisconnectAsync(string reason)
    {
        ...
        await gameManager.OnClientDisconnectAsync(this);
    }
